        ...
        if col0[j][-2:] == "竖井":
            ...
            if col0[j][0:3] == "JCD":
                if (not (pre_name+',' + col0[j]) in pointsNames) and (col2[j] != ""):
                    ...
                    currentSpeeds = []
                    sumSpeeds = []
                    waveStart = random.randint(20, 30)  # 随机一个开始波动的数
                    for n in list(range(1, 57)):  # 56个随机数
                        ran = 0.0
                        if n < waveStart:
                            preWave=random.randint(1,10)
                            if(preWave<8):
                                ran = round(0 + random.random() * 0.8, 2)
                            else:
                                ran = round(-0.8 + random.random() * 0.8, 2)
                        else:
...
for picturePData in picturePDatas:
    ...
    xticks = list(range(0, len(picturePData[0]["x"]), 4))  # 这里设置的是x轴点的位置
    xticks.append(55)
    plt.xlabel('日期')
    plt.xticks(xticks,rotation=60)
    plt.ylabel('变形量（mm）')
    plt.title("龙潭220kv变电站-华月路电力隧道工程竖向位移成果表（地表）\n"+p["name"].split(",")[0])
...
#     t = tables[start_page]
#     table = exeExcel.get_sheet(start_page)
#     pointName = p["name"]
#     print(pointName)
#     print(start_page)
...
#
#     table.write(n, 1, 1)
#     table.write(n, 2, p["start_dat"])
#
#     for j in list(range(1, 57)):
#         table = exeExcel.get_sheet(start_page + j)
#         t = tables[start_page + j]
#         m = getRowNumber(t, pointName)
#         print(m)
#
#         table.write(m, 1, j + 1)
#         table.write(m, 2, "%.04f"%p["start_dat"])
#         table.write(m, 3, "%.04f"%p["lastValues"][j - 1])
#         table.write(m, 4, "%.04f"%p["currentValues"][j - 1])
#         table.write(m, 5, "%.02f"%p["randoms"][j - 1])
#         table.write(m, 6, "%.02f"%p["randomSums"][j - 1])
#         table.write(m, 7, "%.02f"%p["currentSpeeds"][j - 1])
#         table.write(m, 8, "%.02f"%p["sumSpeeds"][j - 1])

# exeExcel.save("out2.xls")
