        ...
        if col0[j][-2:] == "点位":
            ...
            if col0[j][0:1] == "f":
                if (not (pre_name+',' + col0[j]) in pointsNames) and (col2[j] != ""):
                    ...
                    for n in list(range(1, 90)):  # 56个随机数
                        ran = round(-0.5 + random.random() * 1.0, 2)
...
for pictureNeedPoint in pictureNeedPoints:
    ...
    for pictureP in pictureNeedPoint:
        ...
        days=[]
        for d in list(range(pictureStartPage,pictureStartPage+89)):
...
for picturePData in picturePDatas:
    ...
    xticks = list(range(0, len(picturePData[0]["x"]), 8))  # 这里设置的是x轴点的位置
    xticks.append(88)
    plt.xlabel('日期')
...
myworkbook.save("地表修改后-加房屋.xlsx")
